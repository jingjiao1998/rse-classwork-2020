...
earthquakes_data = json.loads(quakes.text)
...
with open('earthquakes_data.json','w') as target:
    target.write(quakes.text)

max_magnitude = 0
index = 0
for i in range(len(earthquakes_data['features'])):
    test = earthquakes_data['features'][i]['properties']['mag']
    if test > max_magnitude:
        max_magnitude = test
        index = i
coords = earthquakes_data['features'][index]['geometry']['coordinates']
...
print(f"and it occured at coordinates {coords}.")
